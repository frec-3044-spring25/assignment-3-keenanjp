...
plot2 <- ggplot(co2, aes(x = month, y = monthly_average))+
  geom_point()
plot2
```
...
globaltemp|> 
  plot(Year, Rolling_avg_5, col = 'red')

lines(global_tempclean$Year, global_tempclean$Rolling_avg_10, col = 'blue')

#lines(globaltemp$Year, globaltemp$Rolling_avg_20, col = 'green')

#when I search for the column name it says that it can't find it even though its in the dataset. 
...
icecoretable <- read_table('https://data.ess-dive.lbl.gov/catalog/d1/mn/v2/object/ess-dive-457358fdc81d3a5-20180726T203952542', col_names = FALSE)
colnames(icecoretable) <- c("Depth(m)", "Mean_of_the_ice", "Mean_age_of_the_air", "Co2_concentration")
icecoretable <- print(icecoretable, row.names = FALSE)
icecoretable <- icecoretable[-c(1:16),]
 
#having trouble reading in the dataset
